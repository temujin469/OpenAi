...
function BotHeader() {
  ...
  const handleChange = (e: any) => {
    ...
    notification.open({
      ...
      duration: 3,
      description: `${
        e.target.value === "text-davinci-003"
          ? '"text-davinci-003" модел нь код бичих болон орчуулга хийх чадвартай'
          : `Та яаг одоо "${e.target.value}" моделийг ашиглаж байна`
      }`,
    });
  };
  ...
  const {
    ...
    const { data: response }: { data: { models: [] } } = await baseUrl.get(
      "/bot/models"
    );
      ...
      <ul className="flex justify-between items-center h-full">
        ...
        <li>
          <select
            ...
            className={`dark:bg-secondDarkBg bg-secondBg w-[200px] outline-none p-2 rounded-full dark:text-mainDarkText text-white ${
              location.pathname === "/bot" ? "block" : "hidden"
            }`}
          >
