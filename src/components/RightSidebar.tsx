...
function RightSidebar() {
  ...
  return (
    <div
      className={`flex flex-row z-50 select-none duration-100 w-full md:w-auto fixed ${show ? "right-0" : "right-[-100%]"}`}>
      <div className='shadow-xl flex flex-col z-10  bg-mainBg dark:bg-mainDarkBg p-4 h-screen w-[260px] md:w-[350px]'>
        <div className='flex justify-between mb-10'>
          ...
          <MdOutlineClose onClick={() => setShow(false)} className="md:hidden text-white dark:text-mainDarkText h-10 w-10 p-2 rounded-full shadow-lg" />
        </div>
      ...
      <div className="left-0 top-0 w-full h-screen bg-black/50 absolute animate-none" onClick={() => setShow(false)}>
      </div>
    </div>
  )
}
